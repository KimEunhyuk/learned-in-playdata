...
//			String applicant_id = request.getParameter("applicant_id");
			...
			int bda = Integer.parseInt(request.getParameter("bda"));
			int ipe = Integer.parseInt(request.getParameter("ipe"));


			CertificateDTO certificate = new CertificateDTO(applicant_id, sqld, adsp, dasp, bda, ipe);
		...
		//			request.setAttribute("certificate", certificate);
					...
					url = "certificateSuccess.jsp";	// *** 제대로 applicant insert를 했을 때 url 수정해줌
				}else{
					request.setAttribute("errorMsg", "다시 시도하세요");
				}
			}catch(Exception s){
