...
4. 조인 종류 
	...
	4. outer 조인 
		: 조인시 조인조건이 불충분해도 검색하능하게 하는 조인
		: 두개 이상의 테이블이 조인될때 특정 데이터가 모든 테이블에 존재하지 않고 
			컬럼은 존재하나 null값을 보유한 경우
		  검색되지 않는 문제를 해결하기 위해 사용되는 조인
		  null 값이기 때문에 배제된 행을 결과에 포함 할 수 있드며 (+) 기호를
		   조인 조건에서 정보가 부족한 컬럼쪽에 적용
...
select ename, empno, loc, e.deptno 또는 d.deptno 로 고쳐야함

-- * : 와일드카드, 에스터리스크, 아스타, 별
-- 3. deptno가 동일한 모든 데이터(*) 검색
-- emp & dept 
-- 존재하는 각 table의 모든 컬럼 만큼 기준점 없이 검색 select * from emp, dept;
-- 논리적인 오류
...
7369
select ename, empno, sal, comm, e.deptno, loc from emp e, dept d
...
select m.ename
from emp e, emp m
where e.ename = 'SMITH' and e.deptno = m.deptno;
...
select m.ename
from emp m
where m.deptno = 20 and m.ename != 'SMITH';
...
-- setp01 : 상사가 없는 KING의 이름 검색이 안되었음 !!
...
사원 table 관점에서 mgr이 없는사람 - KING

null이라는 사번의 상사는 있나요? 없음 = 상사 table의 사번에는 null값 없음

사원의 상사는 null이 존재 / 상사의 사번에는 null이 없음

결론 : null에 해당하는 정보 자체가 없는 table은 상사 table
	- 상사 table의 empno는 null없음
*/ 
select e.ename, m.ename 상사명
from emp e, emp m
where e.mgr = m.empno(+);
...
empno가 없는 deptno 40 을 뽑을건데 deptno에는 empno가 없으니까
emp테이블과 조인해서 empno와 deptno40이 한 로우에 있을수 있게함
...
departments 에는 null이라는 department_id 값이 없기 때문에 null이라는 id없는
table 컬럼쪽에 + 기호 표기
*/
...
-- SCOTT/TIGER
--18. KING을 포함하여 관리자가 없는 사원도 모두 검색
-- 단 사원 번호를 기준으로 오름차순 정렬
