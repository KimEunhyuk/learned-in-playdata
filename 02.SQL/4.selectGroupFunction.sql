...
select distinct count(job) from emp;	 안됨
...
*/
-- 부서별 커미션 받는 사원수 
select deptno, count(comm) from emp;  오류!  deptno는 여러개의로우, count는 하나의로우로 오류남
-- 함수를 미 적용하는 컬럼까지 검색시에는 반드시 group by 절에 명시 필수!
select deptno, count(comm) from emp group by deptno;

-- deptno 오름차순(acs) 정렬 추가
-- 존재하는 table에서 부서별 그룹핑 후에 부서별
-- 커미션 개수 파악 후에 부서번호 정렬
select deptno, count(comm) from emp group by deptno order by deptno;
...
group by deptno
having 평균 >= 2000;
-- having 이 select 보다 느려서 평균 >= 2000 을 인식하지 못함
