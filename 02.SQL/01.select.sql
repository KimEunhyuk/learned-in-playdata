...
-- 11. ? dept table의 deptno 값만 검단 오름차순으로 검색
    select deptno from dept order by deptno asc;
-- 12. ? 입사일(hiredate) 검색
-- 입사일이 오래된 직원부터 거색되게 해주세요
    select hiredate as 입사일 from emp order by 입사일 asc;
...
-- 해결책 : null을 0값으로 대체
-- 모든 db는 지원하는 내장 함수
-- oracle에는 null -> 숫자값으로 대체하는 함수 : nvl(null보유컬럼, 대체값)
...
-- 18. 부서번호가 10번 부서의 직원들 이름, 사번, 부서번호 검색  //01-04
-- 단, 사번은 내림차순 검색
/* 10번 부서조건 : where/ ename, empno, deptno / 내림차순(order by ? decs)
*/
select deptno from emp;
...
select ename, empno, deptno from emp where deptno=10 order by ename desc;

--20. 급여가 900이상인 사원들이름, 사번, 급여 검색
...
-- 대소문자 구분없이 검색을 위한 해결책(대소문자 호환 함수)
-- upper() : 소문자 -> 대문자 / lower() : 대문자 -> 소문자
/* 함수 - 독립적으로 개발되는 기능 / 참조 변수없이 함수명만을 호출
메도스 - 클래스 내부에 구현되는 기능 / 객체 참조 변수로 호출*/
select ename, empno, deptno, job from emp where deptno=10 and job=upper('manager');
...
select ename, sal from emp where sal <= 2000 or sal >= 3000 order by sal asc;
...
--25. ? comm이 300 or 500 or 1400이 아닌 사원
select ename, comm
from emp
where comm not in(300,500,1400);   /    where not comm in(300,500,1400);
...
-- date 타입의 yy mm dd의 구분자는 / - 두개 다 됨
select ename, empno, hiredate
from emp
where hiredate='81-09-28';
