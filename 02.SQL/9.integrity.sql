...
select * from emp02;
insert to emp02 (ename) values('유재석');
-- emp02는 empno엔 절대 null 값 불허
select * from emp02;

-- insert to emp02 (ename) values('유재석'); 에러
...
/*
ERROR at line 1:
ORA-00001: unique constraint (SCOTT.SYS_C007048) violated
*/
select * from emp02;
...
-- empno는 pk(제약조건 이름 명시) / ename은 not null(제약조건명 불명시)
...
--? emp02_empno_pk 이름의 정보는 user_constraints 라는 딕셔너리 table에 저장, 
--	이 table 에서 해당 정보가 삭제되는 시점? table이 drop 될때 자동 삭제
...
insert into emp02 values(2, '재석', 70);
/*
ERROR at line 1:
...
insert into emp02 values(1, '재석', 10);
insert into emp02 values(2, '재석', 60); --에러 부모엔 60 데이터 무
...
-- (SCOTT.EMP01_DEPTNO_FK) violated - child record found
...
insert into emp01 values('master', 'M');
insert into emp01 values('master', 'T'); -- 오류
select * from emp01;
